...
Today is the starting view for pending reviews and follow-ups. Deadlines use the local date; Done and Cancelled items appear in History and can be reopened. Search and category/priority/status filters narrow the inbox table using the latest human-confirmed labels when present. Message selectors show ID and subject preview. The review desk defaults to **Lowest confidence first**, using the smaller category/priority estimate with ascending ID as the tie-breaker; **Newest first** is also available. **Include confident predictions** exposes all messages and correction history. **Save human correction** records feedback; **Save and next** moves to the first remaining unresolved message in the current category, priority, batch and sort order only after a successful save and announces completion when the queue is exhausted. Failed saves retain the selected message and form values.
